...
description: "Advanced URDF capabilities including transmissions, Gazebo integration, and Xacro macros"
sidebar_position: 3
...
Advanced URDF features extend the basic robot description capabilities to include simulation-specific elements, parameterization through Xacro, and control interface definitions. These features enable more sophisticated robot models suitable for complex simulation and control scenarios.

## Xacro Parameterization

Xacro (XML Macros) allows parameterization and macro definitions in URDF files, making them more maintainable and reusable.
...
<robot xmlns:xacro="http://ros.org/wiki/xacro" name="conditional_robot">
  ...
  <xacro:macro name="sensor_mount" params="sensor_enabled sensor_type *origin">
    <xacro:if value="${sensor_enabled}">
      ...
      <!-- Sensor-specific elements based on type -->
...
<robot xmlns:xacro="http://ros.org/wiki/xacro" name="math_robot">
  ...
  <!-- Use calculated values for wheel positions -->
...
Transmissions define the interface between ROS control and the physical joints, enabling precise control of robot actuators.
...
<robot name="transmission_robot" xmlns:xacro="http://ros.org/wiki/xacro">
  ...
  <!-- Simple transmission for position control -->
...
```xml
<!-- Complex transmission for mechanisms like grippers -->
...
Gazebo-specific elements extend URDF for physics simulation, sensors, and visualization.
...
```xml
<!-- Custom physics properties for specific links -->
...
## Gazebo Controllers and Plugins

### ROS Control Plugin Configuration

```xml
<!-- Complete robot with ROS control -->
<gazebo>
...
```xml
<!-- Custom plugin for specialized behavior -->
<gazebo>
...
```xml
<!-- Define materials with textures -->
...
class URDFValidatorNode(Node):
    ...
    def validate_kinematic_chain(self, robot):
        """Validate that the robot has a proper kinematic tree"""
        # Check that there's exactly one root link (no parent)
        child_links = set()
...
Advanced URDF features including Xacro parameterization, transmissions, and Gazebo integration provide the tools needed to create sophisticated, reusable, and simulation-ready robot models for complex robotic applications.